  ...
  useEffect(() => {
    document.addEventListener("mousemove", (event) => {
      ...
      if (horCursor.current) {
        horCursor.current.style.transform = `translate3d(0px, ${
          mouseY - horCursor.current.clientHeight / 2 - 30
        }px, 0)`;
      }

      if (verCursor.current) {
        verCursor.current.style.transform = `translate3d(${
          mouseX - verCursor.current.clientWidth / 2 - 30
        }px, 0px, 0`;
      }

      // if (projectContainer.current) {
      //   projectContainer.current.style.width = `${
      //     mouseX - verCursor.current.clientWidth / 2 - 20
      //   }px`;
      // }

      if (projectContainer.current) {
        projectContainer.current.style.transform = `translate3d(${
          mouseX - verCursor.current.clientWidth / 2
        }px, 0px, 0`;
      }
    });
    return () => {};
    ...
    <>
      ...
      <div className="cross hor" ref={horCursor}>
        {width > 500 && props.isActive ? (
          <>
            <div
              className="standard-container projectnamecontainer background"
              ref={projectContainer}
            >
              {props.selectedProject.title && (
                <p ref={title} id="title" className="projectTitle borderBottom">
                  {props.selectedProject.title}
                </p>
              )}
              {props.selectedProject.message != null && (
                <p>{props.selectedProject.message}</p>
              )}
            ...
            {/* {props.selectedProject.year && (
              <p className="standard-container projectYear background">
                {props.selectedProject.year}
              </p>
            )} */}
          </>
