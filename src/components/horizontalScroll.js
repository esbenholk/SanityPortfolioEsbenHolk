...
export class HorizontalScrollComp extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      disableScroll: false,
  ...
  handleWheel(e) {
    ...
    if (this.scrollContainerRef) {
      this.scrollContainerRef.current.scrollLeft += e.deltaY;
    }
  }
        ...
        >
          ...
          {this.props.images.map((image, index) => (
            <Card
              title={"card" + index}
              image={image}
              classes={index === 0 ? "is-clone is-start" : "is-clone"}
              height={this.state.height}
            />
          ))}
        </div>
