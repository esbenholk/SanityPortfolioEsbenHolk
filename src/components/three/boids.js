...
import ProjectButtonList from "../projectButtonList";
import MobileUnfoldingMenu from "../projectButtonMobile";
...
const OscillatingImagePlanes = forwardRef(
  (
    { isMobile, project, stateChanger, camera, onClick, currentProject, image },
    ref
  ) => {
    ...
    return (
      <>
        {Array.from({ length: 5 }).map((_, i) => (
          <group key={i}>
            <OscillatingImagePlane
              isMobile={isMobile}
              upgradeImageOnClick={upgradeImageOnClick}
              texture={upgradableTexture}
...
const OscillatingImagePlane = ({
  ...
  const [isCenteredInFocus, setIsCenteredInFocus] = useState(false);
  ...
  useFrame((delta) => {
    ...
    if (meshRef.current) {
      const targetPosition = movingToCenter
        ? camera.position
      ...
      if (
        meshRef.current.position.distanceTo(targetPosition) < distanceTOcenter
      ) {
    ...
    (e, value) => {
      ...
      if (width > 600) {
        if (value) {
          ...
          window.hoverProject(project);
        } else {
          // stateChanger(null);
          window.unhoverProject(project);
        }
      }
    },
    [project, width]
  ...
  const onMissed = useCallback(
    (e) => {
      e.stopPropagation();
      // setIsActive(false);
      setIsCenteredInFocus(false);
      onClick(null);
      // window.deactivateActive();
      // setIsClicked(false);
      if (width > 600) {
        window.closeAllButtons();
      }
    },
    [onClick, width]
  );
...
const ImageCubeScene = ({
  projects,
  stateChanger,
  handlePlaneClickOutsideCanvas,
}) => {
  const controlsRef = useRef();
  const { camera } = useThree();
  const planeClusterRefs = useRef(new Map());

  const registerRef = (id, ref) => {
    if (ref) planeClusterRefs.current.set(id, ref);
  };
  ...
  const setCurrentProjectFromButtons = (buttonProject) => {
    setCurrentProject(buttonProject);
    upgradeByProjectId(buttonProject.title);
  };
  ...
  window.setCurrentProjectFromButtons = setCurrentProjectFromButtons;
...
export default function Boids({
  ...
  const handlePlaneClick = (_clickedProject) => {
    if (_clickedProject) {
      // focusProject(_clickedProject.slug.current);
      if (width > 600) {
        window.focusProject(_clickedProject);
      }
      if (width < 600) {
        setClickedProject(null);
      }

      setClickedProject(_clickedProject); // Set the camera's target position to focus on the clicked plane
    } else {
      setClickedProject(_clickedProject);
    }
  };
    ...
    <>
      <Canvas camera={{ position: [0, 0, 0.1] }} className="mainCanvas">
        <ImageCubeScene
          ...
          handlePlaneClickOutsideCanvas={handlePlaneClick}
        />
      </Canvas>
      {width > 600 ? (
        <ProjectButtonList projects={projects} />
      ) : clickedProject != null ? (
        <>
          <MobileUnfoldingMenu clickedProject={clickedProject} />;
        </>
      ) : null}
